...
Seed-v1 renders with the same phase/week/day order, stable `wNNdN` progress keys, curriculum copy, content/part links, stars, progress controls, and ritual links as the static source. Intentional presentation-only DOM differences are: calendar content is now built with text nodes instead of HTML strings; cards carry `data-unit-key` for focus restoration; and a future `availability="missing"` unit gets an explicit **Unavailable** badge, an `aria-disabled` title/content region referenced by the card's `aria-describedby` (ritual links stay outside it and remain active), muted/dashed styling, and no curriculum/progress link. The narrow mobile rules wrap the navigation and calendar at 320 px without changing curriculum meaning.
