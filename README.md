...
Fresh credential-free browser evidence covers 320 px and desktop layouts: canonical assistant parity and divergent-assignment suppression; prompt active-request cancellation with one connected fallback remount; measured effective light/dark contrast for progress, milestone, title-hover, ritual, and availability states; successful and failed-retry focus restoration; Lighthouse accessibility; maximum-length labels; nullable colors; hostile hrefs; and a synthetically blocked loader asset. Results are recorded in `tests/evidence/curriculum-correction-browser.txt`.
