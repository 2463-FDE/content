...
Seed-v1 renders with the same phase/week/day order, stable `wNNdN` progress keys, curriculum copy, content/part links, stars, progress controls, and ritual links as the static source. A backend `subtitle: null` is normalized to empty display text. Intentional presentation-only DOM differences are: calendar content is now built with text nodes instead of HTML strings; cards carry `data-unit-key` for focus restoration; `availability="missing"` gets an explicit **Unavailable** badge with muted/dashed styling; and backend-valid `availability="planned"` gets a distinct **Planned** badge with dotted styling. Both unavailable states put `aria-disabled` on the title/content region referenced by the card's `aria-describedby`, expose no curriculum/part/progress action, and keep ritual links outside that region and active. The narrow mobile rules wrap the navigation and calendar at 320 px without changing curriculum meaning.
