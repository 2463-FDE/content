...
**Generation — Claude on Bedrock via the Converse API.** Default to **Claude Haiku 4.5** (`us.anthropic.claude-haiku-4-5-20251001-v1:0`) for cost; size up to **Claude Sonnet 5** (`us.anthropic.claude-sonnet-5`, verified 2026-09-25 per `docs/research/w02/w02d5.md`) only where quality demands it (e.g. the Ragas judge). Geo profiles can route outside the source Region; they do not guarantee single-Region residency. Claude 3.5 is retired on Bedrock. These current-gen models require an **inference-profile ID** (the `us.` / `global.` prefix), not the bare `anthropic.` id. Same bearer-token auth; keeps the Claude-centric framing under the all-Bedrock mandate.
