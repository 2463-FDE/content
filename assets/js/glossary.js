...
(function () {
  ...
  var FDE_GLOSSARY = [
    ...
    { term: "XY problem", def: "The client asks for their guessed fix (Y) instead of the problem they actually have (X).", aka: ["XY"] },
    { term: "shelfware", def: "Software that was built and shipped and then never used by anyone." },
    { term: "feature-parity trap", def: "Rebuilding every old feature in new tech — usually the most expensive way to modernize.", aka: ["feature parity", "feature-parity"] },
    { term: "target state", def: "The agreed picture of what the modernized system will be, chosen before the spec is written." },
    { term: "strangler fig", def: "Growing the new system around the old one and moving behavior across a slice at a time.", aka: ["strangler-fig", "strangler"] },
    { term: "idempotency key", def: "A client-supplied id that lets a server recognize a retry and not do the work twice.", aka: ["idempotency keys"] },
    { term: "context graph", def: "A map of how decisions get made in a system — its business rules, standards, and constraints.", aka: ["context graphs"] },
    { term: "stopping condition", def: "The machine-checkable test that tells an automated loop it is finished.", aka: ["stopping conditions"] },
