...
(function () {
  ...
  // ---- Curriculum days ----------------------------------------------------
  // The daily interview is scoped to material taught through the chosen day
  // (cumulative). Backend scopes at week granularity until the bank is
  // day-tagged; the day value is forward-compatible. Extend as weeks are
  // authored. DEFAULT_DAY = the cohort's latest available day.
  var CURRICULUM_DAYS = [
    { v: "w01d1", t: "Week 1 · Day 1" },
    { v: "w01d2", t: "Week 1 · Day 2" },
    { v: "w01d3", t: "Week 1 · Day 3" },
    { v: "w01d4", t: "Week 1 · Day 4" },
    { v: "w01d5", t: "Week 1 · Day 5" },
    { v: "w02d1", t: "Week 2 · Day 1" },
    ...
    { v: "w04d1", t: "Week 4 · Day 1" },
    { v: "w04d2", t: "Week 4 · Day 2" },
    { v: "w04d3", t: "Week 4 · Day 3" },
    { v: "w04d4", t: "Week 4 · Day 4" },
    { v: "w04d5", t: "Week 4 · Day 5" },
    { v: "w05d1", t: "Week 5 · Day 1" },
    { v: "w05d2", t: "Week 5 · Day 2" },
    { v: "w05d3", t: "Week 5 · Day 3" },
    { v: "w05d4", t: "Week 5 · Day 4" },
    { v: "w05d5", t: "Week 5 · Day 5" },
    { v: "w06d1", t: "Week 6 · Day 1" },
  ...
  // Shared 2463 identity, written by the program site's auth.js as
  // localStorage["fde_identity"] = {code, name, role}. The `code` doubles as the
  // gauntlet passcode, so a learner who signed in on the program site never sees
  // a second login.
  function readFdeIdentity() {
    try {
      var idt = JSON.parse(localStorage.getItem("fde_identity") || "null");
      return (idt && idt.code) ? idt : null;
    } catch (e) { return null; }
  }

  // Auto-authenticate from the shared identity and jump straight to the hub.
  // Falls back to the (single access-code) gate on any failure.
  function autoLogin(pid) {
    renderHeader();
    var root = app();
    clear(root);
    root.appendChild(el("div", { class: "screen screen--center" }, [
      el("div", { class: "card gate-card screen-in" }, [
        el("p", { class: "gate-sub", text: "Signing you in as " + (pid.name || "your account") + "…" })
      ])
    ]));
    window.API.session("", pid.code)
      .then(function (data) {
        if (data && data.ok) {
          state.name = data.name;
          state.passcode = pid.code;
  ...
  function renderGate() {
    ...
    // Identity comes from the shared 2463 access code. If the learner already
    // signed in on the program site (auth.js wrote localStorage.fde_identity),
    // boot() auto-logs them in and this gate never shows. This is the fallback
    // for a direct visit with no prior identity: one access-code box. The code
    // IS the passcode — the backend resolves the name from it and ignores any
    // client-sent name.
    ...
    var code = el("input", {
      ...
      id: "codeInput",
      type: "password",
      placeholder: "Your 2463 access code",
      autocomplete: "off"
    });
    ...
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var passcode = code.value.trim();
      if (!passcode) { window.showToast("Enter your access code.", "warn"); return; }
      btn.disabled = true;
      btn.textContent = "Checking…";
      window.API.session("", passcode)
        .then(function (data) {
          if (data && data.ok) {
            state.name = data.name;        // server is authoritative
    ...
    var card = el("div", { class: "card gate-card screen-in" }, [
      ...
      el("p", { class: "gate-back" }, [
        el("a", { href: "index.html", class: "gate-back__link", text: "← Back to the 2463 program" })
      ])
    ]);
